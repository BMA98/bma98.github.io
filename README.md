...
of JavaScript. A corner toggle links each page to the other, and matching
`hreflang` tags in both heads tell search engines they are translations rather
than duplicates.
...
files differ only in their `canonical` tag and in which side of the toggle is the
link, so `diff index.html es/index.html` is a practical way to see what is out of
sync.
